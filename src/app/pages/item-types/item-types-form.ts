...
import { ItemTypeStatus, ManagedItemType, SaveItemTypePayload } from './item-types.service';

export type ItemTypeFormMode = 'create' | 'edit' | 'detail';
    ...
    template: `
        <form [formGroup]="form" (ngSubmit)="submit()" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 flex flex-col gap-5">
                ...
                @if (mode !== 'detail') {
                    <p-message severity="info" text="Los tipos inactivos no deberian seleccionarse en nuevos items." />
                }
            </div>
                ...
                @if (itemType) {
                    <div class="border border-surface-200 dark:border-surface-700 rounded-lg p-4 flex flex-col gap-3">
                        <div>
                            ...
                            <div class="mt-1"><p-tag [value]="statusLabel(itemType.status)" [severity]="itemType.status === 'active' ? 'success' : 'danger'" /></div>
                        </div>
                        <div>
                            <span class="text-sm text-muted-color">Items asociados</span>
                            <p class="font-medium">{{ itemType.items_count }}</p>
                        </div>
                        <div>
                            <span class="text-sm text-muted-color">Creado</span>
                            <p class="font-medium">{{ itemType.created_at | date: 'dd/MM/yyyy HH:mm' }}</p>
                        </div>
                        <div>
                            <span class="text-sm text-muted-color">Actualizado</span>
                            <p class="font-medium">{{ itemType.updated_at | date: 'dd/MM/yyyy HH:mm' }}</p>
                        </div>
                    </div>
                } @else {
                    <div class="border border-surface-200 dark:border-surface-700 rounded-lg p-4">
                        <span class="text-sm text-muted-color">Tabla Supabase</span>
                        <p class="font-medium">item_types</p>
                    </div>
                }
            </div>

            <div class="lg:col-span-3 flex justify-end gap-3">
                <p-button type="button" label="Volver" icon="pi pi-arrow-left" severity="secondary" [outlined]="true" routerLink="/item-types" />
                @if (mode !== 'detail') {
...
export class ItemTypesForm implements OnChanges {
    private readonly fb = inject(FormBuilder);

    @Input() mode: ItemTypeFormMode = 'create';
    @Input() itemType: ManagedItemType | null = null;
    @Input() saving = false;
    @Output() save = new EventEmitter<SaveItemTypePayload>();

    readonly statusOptions: { label: string; value: ItemTypeStatus }[] = [
    ...
    readonly form = this.fb.nonNullable.group({
        ...
        status: ['active' as ItemTypeStatus, Validators.required]
    });

    ngOnChanges(changes: SimpleChanges): void {
        if (changes['itemType'] || changes['mode']) {
            this.syncForm();
        }
    }
